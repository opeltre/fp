...
class Method:
    ...
    def __set_name__(self, obj, name):
        obj._methods = obj._methods if '_methods' in dir(obj) else {}
        self.name = name
    ...
    def __get__(self, obj, cls=None):
        ...
        if obj is not None:
            ...
            return self.signature(method)
        return self.signature(Constructor(cls.__name__))
...
class FuncMeta(ConstructorMeta):
    ...
    def methods(F):
        return dict(
            fmap = Method(Hom('A', 'B'), Hom(F('A'), F('B')))
        )
...
class Func(TypeMeta, metaclass=FuncMeta):
    ...
    def name(cls, *As):
        ...
        tail = ", ".join(names)
        return (f'{cls.__name__} ({tail})' if len(names) > 1 else
                f'{cls.__name__} {tail}')
